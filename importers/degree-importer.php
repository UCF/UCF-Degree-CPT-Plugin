...
class UCF_Degree_Importer {
	...
	public function __construct( $search_url, $api_key, $do_writebacks, $additional_params='' ) {
		...
		/**
		 * Allows for the default program types to be overwritten in a child theme or plugin
		 * @author Jim Barnes
		 * @since v3.2.12
		 * @param array $program_types The array of program types
		 *
		 * @return array The transformed array of program types
		 */
		$this->program_types = apply_filters( 'ucf_degree_default_program_types',  array(
...
";
	...
	private function fetch_api_results() {
		...
		/**
		 * General filter for adjusting API results before delivering
		 * them to the rest of the importer.
		 * @author Jim Barnes
		 * @since v3.2.12
		 * @param array $results The results from the API calls
		 *
		 * @return array The transformed results array
		 */
		return apply_filters( 'ucf_degree_api_results', $results );
	}
...
class UCF_Degree_Import {
	...
	private function get_program_types() {
		...
		/**
		 * Filter the allows for specific assignment to program types in child themes/plugins
		 * @author Jim Barnes
		 * @since v3.2.12
		 * @param array $program_types The default program_types generated by this importer
		 * @param string $career The career string pulled from the search service
		 * @param string $level The level string pulled from the search service
		 *
		 * @return array The transformed $program_types array
		 */
		return apply_filters( 'ucf_degree_assign_program_types', $program_types, $this->career, $this->level );
	}
	...
	private function get_post_metadata() {
		...
		if ( has_filter( 'ucf_degree_get_post_metadata' ) ) {
			$existing_id = ( ! $this->is_new ) ? $this->existing_post->ID : null;
			$meta = apply_filters( 'ucf_degree_get_post_metadata', $meta, $this->program, $existing_id );
		}
	...
	private function get_tuition( $type ) {
		...
		if ( $type === 'resident' ) {
			if ( $this->program->resident_tuition ) {
				$amount = '$' . number_format( $this->program->resident_tuition, 2 );
			} else {
				return null;
			}
		} else if ( $type === 'nonresident' ) {
			if ( $this->program->nonresident_tuition ) {
				$amount = '$' . number_format( $this->program->nonresident_tuition, 2);
			} else {
				return null;
			}
		}
