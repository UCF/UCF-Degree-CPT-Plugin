	...
	class UCF_Degree_List_Common {
		public function display_degrees( $items, $layout, $title, $display_type='default', $grouped=false, $groupby_field=null ) {
			...
			if ( has_action( 'ucf_degree_list_display_' . $layout . '_before' ) ) {
				do_action( 'ucf_degree_list_display_' . $layout . '_before', $items, $title, $display_type, $grouped );
			}
...
/**
 * Defines hooks for displaying lists of degrees with the classic layout
 **/

if ( ! function_exists( 'ucf_degree_list_display_classic_before' ) ) {
	function ucf_degree_list_display_classic_before( $items, $title, $display_type, $grouped ) {
		ob_start();
	?>
	...
	add_action( 'ucf_degree_list_display_classic_before', 'ucf_degree_list_display_classic_before', 10, 4 );
}
...
/**
 * Defines hooks for displaying lists of degrees with the count layout
 **/

if ( ! function_exists( 'ucf_degree_list_display_count_before' ) ) {
	function ucf_degree_list_display_count_before( $items, $title, $display_type, $grouped ) {
		return;
	}

	add_action( 'ucf_degree_list_display_count_before', 'ucf_degree_list_display_count_before', 10, 4 );
}

if ( ! function_exists( 'ucf_degree_list_display_count_title' ) ) {
	function ucf_degree_list_display_count_title( $items, $title, $display_type ) {
		return;
	}

	add_action( 'ucf_degree_list_display_count_title', 'ucf_degree_list_display_count_title', 10, 3 );
}

if ( ! function_exists( 'ucf_degree_list_display_count' ) ) {
	function ucf_degree_list_display_count( $items, $title, $display_type ) {
		echo count($items);
	}

	add_action( 'ucf_degree_list_display_count', 'ucf_degree_list_display_count', 10, 3 );
}

if ( ! function_exists( 'ucf_degree_list_display_count_grouped' ) ) {
	function ucf_degree_list_display_count_grouped( $items, $title, $display_type, $groupby_field ) {
		ob_start();
		foreach( $items as $item ) : // For each group
	?>
		<div class="degree-list-group">
			<h3 class="degree-list-heading"><?php echo $item['group_name']; ?></h3>
			<?php echo ucf_degree_list_display_count( $item['posts'], $title, $display_type ); ?>
		</div>
	<?php
		endforeach;
		echo ob_get_clean();
	...
	add_action( 'ucf_degree_list_display_count_grouped', 'ucf_degree_list_display_count_grouped', 10, 4 );
}

if ( ! function_exists( 'ucf_degree_list_display_count_after' ) ) {
	function ucf_degree_list_display_count_after( $items, $title, $display_type ) {
		return;
	}

	add_action( 'ucf_degree_list_display_count_after', 'ucf_degree_list_display_count_after', 10, 3 );
}
